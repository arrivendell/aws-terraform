...
resource "aws_security_group" "instances_default_sg" {
  ...
  description = "Allowing HTTP custom app port and SSH from adriens to host for instances behind elb"
  vpc_id      = "${aws_vpc.url_shortener.id}"
...
resource "aws_security_group" "allow_ec2_instances_sg" {
  name        = "allow_ec2_instances_sg"
  description = "Allow all traffic in from ec2  cluster"
  vpc_id      = "${aws_vpc.url_shortener.id}"
